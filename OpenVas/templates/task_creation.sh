...
HOST_ID_FULl=$(omp -u admin -w admin -h 127.0.0.1 -p 9390 --xml '<create_target><name>local</name><hosts>127.0.0.1</hosts><alive_tests>ICMP, TCP-ACK Service &amp; ARP Ping</alive_tests></create_target>')
...
echo "$(date): Exporting variables to /root/.profile" >> "$LOG_FILE"
echo "export TARGET_ID=$HOST_ID" >> /root/.profile
echo "export IP_TARGET=127.0.0.1" >> /root/.profile
